...
void loop(){
  if (running) {
    if (getTimer(foggerTimmer, foggerWaitInterval) && foggerNotDone)
    {
      
    }   
    if (getTimer(foggerTimmer, foggerWaitInterval + foggerOnInterval) && foggerNotDone)
    {
      foggerNotDone = false;
    }
    ...
    if (getTimer(globalTimer, maxTime)
    {
      foggerDone = true;
      ...
    }
